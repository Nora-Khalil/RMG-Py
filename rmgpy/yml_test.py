...
from datetime import datetime #added in by me 
from rmgpy.chemkin import get_species_identifier  #added in by me 
...
def get_mech_dict(spcs, rxns, solvent='solvent', solvent_data=None):
     ...
     for spc in spcs:
         ...
         if not is_surface:   
             result_dict = dict()
             result_dict["species"] = dict()
             #result_dict["species"][0] = [dict()]
             result_dict["species"][0] = [obj_to_dict(x, spcs, names=names) for x in spcs]
             # result_dict["species"][0]["thermo"] = [obj_to_dict(x, spcs, names=names) for x in spcs]
...
def obj_to_dict(obj, spcs, names=None, label="solvent"):
    ...
    if isinstance(obj, Species):
        result_dict["name"] = obj.to_chemkin()
        result_dict["composition"] = obj.to_cantera().composition
        #result_dict["thermo"] = dict()
    ...
    #     result_dict["model"] = [obj_to_dict(k, spcs) for k in obj.polynomials]
    #     result_dict["type"] = "NASA"
    elif isinstance(obj, NASAPolynomial):
         result_dict["model"] = "NASApolynomial"
         result_dict["data"] = obj.coeffs.tolist()
